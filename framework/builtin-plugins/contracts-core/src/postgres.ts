export type ContractsCoreSqlOptions = {
  schemaName?: string;
  dropSchema?: boolean;
};

export function buildContractsCoreMigrationSql(options: ContractsCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "contracts_core", "schemaName");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getContractsCoreLookupIndexName()} ON ${schemaName}.primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getContractsCoreStatusIndexName()} ON ${schemaName}.exception_records (tenant_id, status, severity);`
  ];
}

export function buildContractsCoreRollbackSql(options: ContractsCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "contracts_core", "schemaName");
  const dropSchema = options.dropSchema ?? schemaName !== "contracts_core";
  return [
...
export function getContractsCoreLookupIndexName(): string {
  return "contracts_core_primary_lookup_idx";
}

export function getContractsCoreStatusIndexName(): string {
  return "contracts_core_exception_status_idx";
}
