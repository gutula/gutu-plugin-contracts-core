export type ContractsCoreSqliteOptions = {
  tablePrefix?: string;
};

export function buildContractsCoreSqliteMigrationSql(options: ContractsCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "contracts_core_");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getContractsCoreSqliteLookupIndexName(tablePrefix)} ON ${tablePrefix}primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getContractsCoreSqliteStatusIndexName(tablePrefix)} ON ${tablePrefix}exception_records (tenant_id, status, severity);`
  ];
}

export function buildContractsCoreSqliteRollbackSql(options: ContractsCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "contracts_core_");
  return [
...
export function getContractsCoreSqliteLookupIndexName(tablePrefix = "contracts_core_"): string {
  return `${normalizePrefix(tablePrefix)}primary_lookup_idx`;
}

export function getContractsCoreSqliteStatusIndexName(tablePrefix = "contracts_core_"): string {
