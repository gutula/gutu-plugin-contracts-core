...
import { businessFlowDefinitions, createContract, activateEntitlement, publishBillingSchedule, placeRecordOnHold, releaseRecordHold, amendRecord, reverseRecord } from "../../src/flows/catalog";
...
describe("plugin manifest", () => {
  it("keeps a stable package id and business contract surface", () => {
    expect(manifest.id).toBe("contracts-core");
    expect(manifest.kind).toBe("plugin");
    expect(manifest.publicCommands).toContain("contracts.registry.create");
    expect(manifest.publicEvents).toContain("contracts.registered.v1");
  });
});
...
describe("domain catalog", () => {
  it("keeps ERPNext parity references and operational surfaces visible", () => {
    ...
    expect(typeof activateEntitlement).toBe("function");
    expect(typeof publishBillingSchedule).toBe("function");
